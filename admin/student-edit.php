                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="form-control select" name="academic_year_id" id="academic_year_id" style="width: 100%;" required>
                            ...
                            <?php
                            $sessionResult = $db_handle->query("SELECT session_name FROM st_session_master ORDER BY session_id DESC");
                            $currentAcademicYear = (string) ($row['academic_year'] ?? '');
                            $academicYearMatched = false;
                            if ($sessionResult) {
                                while ($sessionRow = $sessionResult->fetch_assoc()) {
                                    $sessionName = (string) ($sessionRow['session_name'] ?? '');
                                    $selected = ($currentAcademicYear !== '' && $currentAcademicYear === $sessionName) ? 'selected' : '';
                                    if ($selected !== '') {
                                        $academicYearMatched = true;
                                    }
                                    echo '<option value="' . htmlspecialchars($sessionName, ENT_QUOTES) . '" ' . $selected . '>' . htmlspecialchars($sessionName) . '</option>';
                                }
                            }
                            if ($currentAcademicYear !== '' && !$academicYearMatched) {
                                echo '<option value="' . htmlspecialchars($currentAcademicYear, ENT_QUOTES) . '" selected>' . htmlspecialchars($currentAcademicYear) . '</option>';
                            }
                            ?>
                        </select>
                    </div>
